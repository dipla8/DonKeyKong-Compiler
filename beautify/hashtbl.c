...
int hashtbl_remove(HASHTBL *hashtbl, const char *key,int scope, array_t *arr) {
	...
	while(node) {
		if((!strcmp(node->key, key)) && (node->scope == scope)) {
			free(node->key);
			free(node->data);
			if (node->arr != NULL) {free(node->arr);}
...
int findsize2(struct hashnode_s *n, HASHTBL *hashtbl, int scope){
	...
	if (!strcmp(n->data,"int")) {return sizeof(int)*mul;}
	else if (!strcmp(n->data,"char")) {return sizeof(char)*mul;}
	else if (!strcmp(n->data, "float")) {return sizeof(float)*mul;}
	else {
...
int findsize3(struct hashnode_s *n, HASHTBL *hashtbl, int scope){
	...
	if (!strcmp(n->data,"int")) {return mul;}
	else if (!strcmp(n->data,"char")) {return mul;}
	else if (!strcmp(n->data, "float")) {return mul;}
	else {
		struct hashnode_s *tmp;
		tmp = hashtbl_lookup(hashtbl, scope, n->data, 5);
				...
				if(node->init == NULL) {
					...
					for(int i = 0; i < real_size; i++){
						if(temp != NULL){
							...
							if(node->init->type == T_CHAR) {fprintf(fd, "\\0");}
							else {fprintf(fd, " 0");}
						}
						if(i != real_size - 1) {fprintf(fd, ",");}
					}
					fprintf(fd,"\n");
				}
				oldnode = node;
